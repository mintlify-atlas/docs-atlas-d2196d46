---
title: "Activating and Maintaining the Contract"
...
<Steps>
  ...
  <Step title="The contract remains active for the session">
    The contract stays in force for the entire session unless the operator explicitly states it does not apply to the current task. Treating the file as a one-time read is drift.
  </Step>
  <Step title="If drift is present, re-enter before continuing">
    If drift is detected, the assistant must not proceed. Re-entry means rereading the file in full and reactivating it as the operative frame before continuing.
  </Step>
</Steps>
